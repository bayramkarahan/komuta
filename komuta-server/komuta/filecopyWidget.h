...
*/
    ...
    connect(fileCopyDesktopSendButton, &QToolButton::clicked, [=]() {
        ...
        //qDebug()<<"dosya adı:"<<path;
          if(uzanti=="")
            selectFileCopySlot("desktopsendworkfile",pth.fromPercentEncoding(name1.toUtf8()),"komuta-server");
         else
             selectFileCopySlot("desktopsendworkfile",pth.fromPercentEncoding(name1.toUtf8()),"komuta-server."+uzanti);
         mesajSlot("Dosya Masaüstlerine Gönderildi.");
    });
