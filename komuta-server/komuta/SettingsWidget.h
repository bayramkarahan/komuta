    ...
    connect(webAyarGuncelleButton, &QPushButton::clicked, [=]() {
        ...
        /*for(int i=0;i<onlinePcList.count();i++)
        {
                ...
                onlinePcList[i]->user!="noLogin")
            {
                ...
                selectFileCopySlot("webblockserversendfile",onlinePcList[i]->ip,onlinePcList[i]->netProfil.ftpPort, "/usr/share/komuta/webblockserver.json","webblockserver.json");
                system("sleep 0.1");
             }
        }*/
        selectFileCopySlot("webblockserversendfile", "/usr/share/komuta/webblockserver.json","webblockserver.json");
...
void MainWindow::networkProfil()
{
    ...
    DatabaseHelper *db=new DatabaseHelper(localDir+"komuta.json");
    QJsonArray dizi=db->Oku();
    int sr=0;
...
void MainWindow::networkProfilLoad()
{
    ...
    if(dizi.count()>0)
    {
        ...
        system("systemctl restart komuta-networkprofil.service");
    }
}
...
void MainWindow::bilgiAlButtonSlot()
{
    ...
    doc->setHtml("<center><h2>komuta Hakkında</h2></center>"
                 "<center><img src=\":/icons/komuta.svg\" width=75 height=75 /></center> "
                 ...
                 "<br/><br/>komuta uygulamasını kullanmaktan doğabilecek her türlü hukuki sorumluluğu kullanıcı kabul etmiş sayılır."
